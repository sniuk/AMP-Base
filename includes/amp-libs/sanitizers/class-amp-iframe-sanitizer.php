 ...
 */
class amp_base_iframe_sanitizer extends amp_base_base_sanitizer {
	...
	public function sanitize() {
		...
		for ( $i = $num_nodes - 1; $i >= 0; $i-- ) {
			$node = $nodes->item( $i );
			$old_attributes = amp_base_dom_utils::get_node_attributes_as_assoc_array( $node );
			...
			$new_node = amp_base_dom_utils::create_node( $this->dom, 'amp-iframe', $new_attributes );
			...
			if ( 'p' === strtolower( $parent_node->tagName ) ) {
				...
				if ( amp_base_dom_utils::is_node_empty( $parent_node ) ) {
	...
	private function build_placeholder( $parent_attributes ) {
		$placeholder_node = amp_base_dom_utils::create_node( $this->dom, 'div', array(
			'placeholder' => '',
